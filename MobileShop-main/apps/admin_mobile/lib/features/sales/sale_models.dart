...
///
/// [costAtSale] is the product's cost price *at the moment this sale was
/// made* — added in plan item 1.4 so profit reports no longer have to fall
/// back on the product's *current* cost (which drifts if the cost changes
/// later). [costIsEstimated] is false for every sale created from 1.4
/// onward (the cost is a genuine snapshot); it's true only for sales made
/// before this field existed, where the 1.4 migration backfilled
/// [costAtSale] from whatever the product's cost happened to be at
/// migration time, since the real historical cost was never recorded.
@immutable
class SaleItemRecord {
  ...
  const SaleItemRecord({
    ...
    this.costIsEstimated = false,
  });
  double get lineTotal => quantity * unitPrice;
  double get lineCost => quantity * costAtSale;
}
