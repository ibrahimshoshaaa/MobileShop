...
class SettingsPage extends StatelessWidget { const SettingsPage({super.key}); @override Widget build(BuildContext context)=>ListView(padding:const EdgeInsets.all(16),children:[SwitchListTile(value:false,onChanged:(_){},title:const Text('تفعيل Online'),subtitle:const Text('ربط الحساب بالسيرفر المركزي')),const ListTile(leading:Icon(Icons.account_circle),title:Text('حساب الشركة'),subtitle:Text('تسجيل الدخول وربط الفروع')),const ListTile(leading:Icon(Icons.backup),title:Text('النسخ الاحتياطي'),subtitle:Text('إعدادات النسخ والاسترجاع')),const ListTile(leading:Icon(Icons.security),title:Text('الأمان والصلاحيات'),subtitle:Text('المستخدمون والأدوار وسجل التدقيق'))]); }
