...
AUTH_PROVIDER = os.getenv("AUTH_PROVIDER", "dev").lower()

if APP_ENV == "production" and AUTH_PROVIDER == "dev":
    raise RuntimeError("Refusing to start production with development-token authentication")

app = FastAPI(
    title="Mobile Shop ERP API",
    version="0.3.0",
    description="ERP HTTP command and synchronization boundary.",
)
...
verify_token = verify_production_token if AUTH_PROVIDER == "firebase" else verify_dev_token
...
def health():
    return {
        "status": "ok",
        "mode": APP_ENV,
        "persistence": "turso/libsql" if os.getenv("TURSO_DATABASE_URL") else "sqlite",
    }
...
def list_products_endpoint(request: Request, branch_id: str = "LOCAL_BRANCH"):
    ...
    if "inventory.read" not in claims.get("permissions", ()):
        return JSONResponse({"ok": False, "error": {"code": "FORBIDDEN", "message": "لا توجد صلاحية قراءة للمخزون.", "details": {"permission": "inventory.read"}}}, status_code=403)
...
def query_endpoint(request: Request, entity: str, branch_id: str = "LOCAL_BRANCH", limit: int = 100):
    claims = verify_token(request)
    if not claims:
        return JSONResponse({"ok": False, "error": {"code": "UNAUTHORIZED", "message": "التوثيق مطلوب.", "details": {}}}, status_code=401)
...
async def sync_upload_endpoint(request: Request):
    ...
    def execute(envelope):
        ...
        return dispatch(engine, envelope.get("command"), ctx, **envelope.get("payload", {}))
