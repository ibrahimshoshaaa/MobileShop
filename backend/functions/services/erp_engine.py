...
class ERPCommandEngine:
    ...
    def create_sale(self,command):
        with self._lock:
            ...
            paid_total=sum((p.amount for p in pays),D0)
            if paid_total>total: raise DomainError('INVALID_PAYMENT','إجمالي المدفوعات أكبر من إجمالي الفاتورة.',{})
            if paid_total<total and not command.customer_id: raise DomainError('INVALID_PAYMENT','البيع الآجل يتطلب عميلًا.',{})
