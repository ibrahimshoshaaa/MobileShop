...
def test_shared_connection_keeps_business_write_and_receipt_atomic(tmp_path):
    """Characterize P0: one connection must commit the command and receipt together."""
    db_path = tmp_path / "erp.db"
    engine = DurableERPCommandEngine(db_path)
    sync = SyncProtocol(connection=engine.connection)
    calls = []

    envelope = {
        "command_id": "sale-atomic-1",
        "tenant_id": "tenant-1",
        "branch_id": "b1",
    ...
    def execute(item):
        ...
        context = CommandContext(
            item["command_id"], "user-1", "b1", frozenset({"products.edit"}), "tenant-1"
        )
